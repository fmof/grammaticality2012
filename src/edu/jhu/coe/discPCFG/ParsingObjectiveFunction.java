...
import edu.jhu.coe.PCFGLA.ConditionalTrainer;
import edu.jhu.coe.PCFGLA.ConstrainedHierarchicalTwoChartParser;
import edu.jhu.coe.PCFGLA.ConstrainedTwoChartsParser;
import edu.jhu.coe.PCFGLA.Grammar;
import edu.jhu.coe.PCFGLA.Lexicon;
import edu.jhu.coe.PCFGLA.ParserData;
import edu.jhu.coe.PCFGLA.SimpleLexicon;
import edu.jhu.coe.PCFGLA.SpanPredictor;
import edu.jhu.coe.PCFGLA.StateSetTreeList;
...
//  		this.nGrWeights = nGrWeights;
	  ...
	  protected void loadConstraints(){
  		...
  		for (int tree=0; tree<myTrees.size(); tree++){
  			...
  			if (!doNotProjectConstraints) eParser.projectConstraints(curBlock[i]);
...
//  }
  ...
  public ParsingObjectiveFunction(Linearizer linearizer, StateSetTreeList trainTrees,
    ...
    this.myRegularization = regularization;
	  ...
	  boolean[][][][][] tmp = edu.jhu.coe.PCFGLA.ParserConstrainer.loadData(consName+"-0.data");
...
//	  	if (cons!=null) allowedStates[i%nProcesses].add(ArrayUtil.clone(cons[i]));
  ...
  public double[] getCurrentWeights(){
	...
	/* (non-Javadoc)
	 * @see edu.jhu.coe.classify.ObjectiveFunction#getLogProbabilities(edu.jhu.coe.classify.EncodedDatum, double[], edu.jhu.coe.classify.Encoding, edu.jhu.coe.classify.IndexLinearizer)
	 */
