...
public class SophisticatedLexicon implements java.io.Serializable, Lexicon {
    ...
    private Set<Pair<Integer,Integer>> tagSubSeen = new HashSet<Pair<Integer,Integer>>();
    Set<ProductionTuple> ptSeen = new HashSet<ProductionTuple>();
	...
	Counter<String> sumToOne = new Counter<String>();
	for (int tag=0; tag<wordToTagCounters.length; tag++){
	    String tagState = (String)n.object(tag);
	    if (wordToTagCounters[tag]!=null) {
		for (String word : wordToTagCounters[tag].keySet()){
		    double[] scores = score(word,(short)tag,0,false,false);
		    //System.out.println("In SophisticatedLexion: "+word+" \t"+Arrays.toString(scores));
		    for(int i=0;i<scores.length;i++){
			//System.out.println("In SophisticatedLexicon: for " +tagState+"_"+i+", scores["+i+"]="+scores[i]);
			sumToOne.incrementCount(tagState+"_"+i,scores[i]);
		    }
	...
	System.out.println("THE LEXICON COUNTER: \n"+sumToOne);

  	return sb.toString();
    }

    private String makeMJStyle(String tagState, String word, double[] scores){
	String s="";
	for(int i=0;i<scores.length;i++){
	    if(scores[i]==0) continue;
	    s += scores[i] + " " + tagState + "_"+i + " --> " + word + "\n";
	}
	return s;
    }
	...
	for (int tag=0; tag<wordToTagCounters.length; tag++){
		...
		for (String word : wordToTagCounters[tag].keySet()){
		    ...
		    for(int i=0;i<scores.length;i++){
			if(scores[i]==0) continue;
			s += ((double)(scores[i]/sumToOne.getCount(tagState+"_"+i))) + " " + tagState + "_"+i + " --> " + word + "\n";
		    }
		    sb.append(s);
		}
	    }
     ...
     */
    @SuppressWarnings("unchecked")
	public SophisticatedLexicon splitAllStates(short[] numNonInternalBeforeLatestCouple, int[] offset, int[] counts, boolean moreSubstatesThanCounts, int mode) {
	...
	for(int i=0;i<offset.length;i++){
	    System.out.println("test for offset["+i+"]="+offset[i]);
	}
	...
	for (int tag = 0; tag < wordToTagCounters.length; tag++) {
		...
		for (String word : wordToTagCounters[tag].keySet()) {
			...
			if(substate<numNonInternalBeforeLatestCouple[tag]){
			    int splitFactor = 2;
			    if (newNumSubStates[tag]==numSubStates[tag]) { splitFactor = 1; } 
			    for (int i = 0; i < splitFactor; i++) {
				lexicon.wordToTagCounters[tag].get(word)[substate * splitFactor + i] = (1.f/splitFactor) * wordToTagCounters[tag].get(word)[substate];
			    }
			} else{
			    try{
				lexicon.wordToTagCounters[tag].get(word)[substate + offset[tag]] = wordToTagCounters[tag].get(word)[substate];
			    } catch(Exception e){
				System.err.println("tag="+tag+", word="+word+", substate="+substate+", offset["+tag+"]="+offset[tag]+", lexicon.wTTC.length="+lexicon.wordToTagCounters.length);
				System.exit(-2);
			    }
			}
		    }
	...
	for (int tag = 0; tag < unseenWordToTagCounters.length; tag++) {
		...
		for (String word : unseenWordToTagCounters[tag].keySet()) {
			...
			if(substate<numNonInternalBeforeLatestCouple[tag]){
			    ...
			    if (newNumSubStates[tag]==numSubStates[tag]) { splitFactor = 1; } 
			    for (int i = 0; i < splitFactor; i++) {
				lexicon.unseenWordToTagCounters[tag].get(word)[substate * splitFactor + i] = (1.f/splitFactor) * unseenWordToTagCounters[tag].get(word)[substate];
			    }
			} else{
			    lexicon.unseenWordToTagCounters[tag].get(word)[substate + offset[tag]] = unseenWordToTagCounters[tag].get(word)[substate];
			}
		    }
		}
	    }
	...
	for (int tag = 0; tag < typeTagCounter.length; tag++) {
		...
		if(substate<numNonInternalBeforeLatestCouple[tag]){
		    ...
		    System.out.println("Let's see what happening with tag="+tag+", substate="+substate+", and offset.length="+offset.length);
		    lexicon.typeTagCounter[tag][substate + offset[tag]] = typeTagCounter[tag][substate];
		    lexicon.tagCounter[tag][substate + offset[tag]] = tagCounter[tag][substate];
		    lexicon.unseenTagCounter[tag][substate + offset[tag]] = unseenTagCounter[tag][substate];
		}
	    }
	}
    ...
    public SophisticatedLexicon propagateHypothesizedCouplings(LatentStatistics LS, Set</*Map<Pair<Integer,Integer>, */BerkeleyCompatibleFragment> easyPreterminalCouplingMap, short[] numSigsPerTag, double gamma){
	short[] newNumSubStates = new short[numSubStates.length];
	newNumSubStates[0] = 1; // never split ROOT
	for (short i = 1; i < numSigsPerTag.length; i++) {
	    newNumSubStates[i] = numSigsPerTag[i];
	}
	SophisticatedLexicon lexicon = new SophisticatedLexicon(newNumSubStates, this.smoothingCutoff, smooth, smoother, this.threshold);
	FragmentProbabilityComputer fragmentPC = new FragmentProbabilityComputer(LS, gamma);

	tagSubSeen = new HashSet<Pair<Integer,Integer>>();
	if(ptSeen ==null) ptSeen = new HashSet<ProductionTuple>();
	// copy and alter all data structures
	lexicon.wordToTagCounters = new HashMap[numSubStates.length];
	lexicon.unseenWordToTagCounters = new HashMap[numSubStates.length];
	for (int tag = 0; tag < wordToTagCounters.length; tag++) {
		...
		for (String word : wordToTagCounters[tag].keySet()) {
		    lexicon.wordToTagCounters[tag].put(word, new double[newNumSubStates[tag]]);
		    //I need the fragment, X, and E_B(X)
		    //changed this to newNumSubStates
		    for(int substate = 0; substate < newNumSubStates[tag]; substate++){
			Pair<Integer,Integer> pair = new Pair<Integer,Integer>(tag,substate);
			Pair<Integer,Integer> gen = InternalNodeSet.getGeneratingTagAndSigs(tag,substate);
			int sigToUse = (gen==null || !InternalNodeSet.justAdded(tag,substate))?substate:gen.getSecond().intValue();
			ProductionTuple pt = new ProductionTuple(true);
			pt.addPreterminal(tag,substate,word);
			BerkeleyCompatibleFragment bcf = new BerkeleyCompatibleFragment(pt);
			if(InternalNodeSet.isSubstateInternal(tag,substate)){
			    //System.err.println("tag="+tag+", word="+word+", substate="+substate+", sigToUse="+sigToUse+", bcf="+bcf);		       
			    boolean doesContain = easyPreterminalCouplingMap.contains(bcf);
			    lexicon.wordToTagCounters[tag].get(word)[substate] = doesContain?1.0:0.0;
			    if(doesContain){
				ptSeen.add(pt);
			    if(!tagSubSeen.contains(new Pair<Integer,Integer>(tag,substate))){
				tagSubSeen.add(new Pair<Integer,Integer>(tag,substate));
			    }
			    }
			} else{
			    //System.err.println("tag="+tag+", word="+word+", substate="+substate+", sigToUse="+sigToUse+", bcf="+bcf);		    
			    lexicon.wordToTagCounters[tag].get(word)[substate] = fragmentPC.computeForFragment(wordToTagCounters[tag].get(word)[substate],(short)tag, (short)sigToUse, bcf);
			}
	
			//BerkeleyCompatibleFragment bcf = easyPreterminalCouplingMap.get(pair);
			//System.err.println("tag="+tag+", word="+word+", substate="+substate+", sigToUse="+sigToUse+", bcf="+bcf);
			//I need to get the ORIGINAL tag/sig pair
		    }
		}
	    }
	}
	...
	for (int tag = 0; tag < unseenWordToTagCounters.length; tag++) {
		...
		for (String word : unseenWordToTagCounters[tag].keySet()) {
			...
			if(InternalNodeSet.isSubstateInternal(tag,substate)){
			    //System.err.println("tag="+tag+", word="+word+", substate="+substate+", sigToUse="+sigToUse+", bcf="+bcf);		       
			    boolean doesContain = easyPreterminalCouplingMap.contains(bcf);
			    lexicon.unseenWordToTagCounters[tag].get(word)[substate] = doesContain?1.0:0.0;
			    if(doesContain){
				//System.err.println("is this contained?");
				ptSeen.add(pt);
			    if(!tagSubSeen.contains(new Pair<Integer,Integer>(tag,substate))){
				tagSubSeen.add(new Pair<Integer,Integer>(tag,substate));
			    }
			    ...
			    lexicon.unseenWordToTagCounters[tag].get(word)[substate] = fragmentPC.computeForFragment(unseenWordToTagCounters[tag].get(word)[substate],(short)tag, (short)sigToUse, bcf);
			}
		    }		    
		}
	    }
	}
	lexicon.tagSubSeen = tagSubSeen;
	lexicon.ptSeen = ptSeen;
	lexicon.totalWordTypes = totalWordTypes;
	lexicon.totalTokens = totalTokens;
	lexicon.totalUnseenTokens = totalUnseenTokens;
	lexicon.totalWords = totalWords;
	...
	for (int tag = 0; tag < typeTagCounter.length; tag++) {
		...
		int sigToUse = (gen==null || !InternalNodeSet.justAdded(tag,substate))?substate:gen.getSecond().intValue();
		if(InternalNodeSet.isSubstateInternal(tag,substate)){
		    boolean wasseen = tagSubSeen.contains(new Pair<Integer,Integer>(tag,substate));
		    lexicon.typeTagCounter[tag][substate] = wasseen?1.0:0.0;
		    lexicon.tagCounter[tag][substate] = wasseen?1.0:0.0;
		    lexicon.unseenTagCounter[tag][substate] = wasseen?1.0:0.0;
		} else{
		    lexicon.typeTagCounter[tag][substate] = fragmentPC.computeForPreterminal(typeTagCounter[tag][sigToUse],(short)tag, (short)sigToUse); //(1.f/splitFactor)*typeTagCounter[tag][substate];
		    lexicon.tagCounter[tag][substate] = fragmentPC.computeForPreterminal(tagCounter[tag][sigToUse],(short)tag, (short)sigToUse);//(1.f/splitFactor)*tagCounter[tag][substate];
		    lexicon.unseenTagCounter[tag][substate] = fragmentPC.computeForPreterminal(unseenTagCounter[tag][sigToUse],(short)tag, (short)sigToUse);//(1.f/splitFactor)*unseenTagCounter[tag][substate];
		}
	    }		    
	}
	...
	//System.out.println("this may be ugly: "+ ptSeen);
	...
	for (int substate=0; substate<numSubStates[tag]; substate++) {
		...
		ProductionTuple pt = new ProductionTuple(true);
		pt.addPreterminal(tag,substate,word);
		//System.err.println("pt="+pt+", and is in ptSeen="+ptSeen.contains(pt));
		if(ptSeen.contains(pt)){
		    //System.out.println("it says that it's properly setting tag="+tag+", substate="+substate+", for word="+word);
		    resultArray[substate] = 1.0;
		}
		continue;
	    }
		...
		//if(p_T==0 || p_W==0){ System.err.println("WHOA! one of these is zero!!! "+p_T+" or "+p_W+" :(");}
		...
		//if(pb_T_S==0){System.out.println("hmm: pb_T_S is zero "+ pb_T_S+", "+ c_TS+", "+p_T_U+", "+c_S);}
    		double p_T = c_Tseen==0?(1.0/total):(c_Tseen / total);
    		double p_W = 1.0 / total;  
		//if(p_T == 0 || p_W==0){System.err.println("WHOA! one of these is zero :( "+ p_T+", "+p_W);}
	...
	//System.out.println("for word="+word+", tag="+tag+", resultArray before = "+ Arrays.toString(resultArray));
	smoother.smooth(tag,resultArray);
	//System.out.println("for word="+word+", tag="+tag+", resultArray after = "+ Arrays.toString(resultArray));
	...
	newLexicon.tagSubSeen = tagSubSeen;
	newLexicon.ptSeen = ptSeen;
    ...
    /* (non-Javadoc)
     * @see edu.jhu.coe.PCFGLA.Lexicon#getLinearizedLexicon()
     */
    ...
    /* (non-Javadoc)
     * @see edu.jhu.coe.PCFGLA.Lexicon#getLinearIndex(java.lang.String, int)
     */
    ...
    /* (non-Javadoc)
     * @see edu.jhu.coe.PCFGLA.Lexicon#clearMapping()
     */
    ...
    /* (non-Javadoc)
     * @see edu.jhu.coe.PCFGLA.Lexicon#scoreSignature(java.lang.String, int, int)
     */
    ...
    /* (non-Javadoc)
     * @see edu.jhu.coe.PCFGLA.Lexicon#scoreWord(java.lang.String, int)
     */
