...
public class HierarchicalUnaryRule extends UnaryRule {
	...
	scoreThisLevel[0][0] = Math.log(b.getScore(0,0));
	...
	double[][] scores = new double[newCStates][newPStates];
	for (int level=0; level<=lastLevel; level++){
		...
		for (int parent=0; parent<newPStates; parent++){
		    scores[child][parent] += scoresThisLevel[child/divisorC][parent/divisorP];
		}
	    }
	}
	for (int child=0; child<newCStates; child++){
	    for (int parent=0; parent<newPStates; parent++){
		scores[child][parent] = Math.exp(scores[child][parent]);
	    }
	}
	setScores2(scores);
    }
