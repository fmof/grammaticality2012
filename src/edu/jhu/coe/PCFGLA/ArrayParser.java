...
import edu.jhu.coe.discPCFG.Linearizer;
...
public class ArrayParser implements Parser {
	...
	*/
    ...
    Tree<String> extractBestViterbiDerivation(Tree<StateSet> tree, int substate, boolean outputScore, boolean useFullLabel){
	//System.out.println("for tree " + tree + "\n\tlabel is " + tree.getLabel() + ", " + tree.getLabel().getState() + " and substate is " + substate);
  	...
  	if (tree.isPreTerminal()){
	    ...
	    child.add(extractBestViterbiDerivation(tree.getChildren().get(0),-1,outputScore, useFullLabel));
	    String goalStr;

	    if(useFullLabel){
		goalStr = tagNumberer.object(tree.getLabel().getState())+"_"+substate;
	    } else{
		goalStr = tagNumberer.object(tree.getLabel().getState())+"-"+substate;
	    }	    
	...
	case 1:
	    ...
	    //	    if(childIndex==-1)
	    //	System.err.println("WHAHAHAHAHAA unary " + tree +", " +child);
	    newChildren.add(extractBestViterbiDerivation(children.get(0), childIndex, outputScore, useFullLabel));
	    break;
	case 2:
	    StateSet leftChild = children.get(0).getLabel();
	    StateSet rightChild = children.get(1).getLabel();
	    ...
	    newChildren.add(extractBestViterbiDerivation(children.get(0), lChildIndex, outputScore, useFullLabel));
	    newChildren.add(extractBestViterbiDerivation(children.get(1), rChildIndex, outputScore, useFullLabel));
	    break;
	default:
	    throw new Error ("Malformed tree: more than two children");
	}
	String parentString = ((String)tagNumberer.object(node.getState()));
	if (!useFullLabel && parentString.endsWith("^g")) 
	    parentString = parentString.substring(0,parentString.length()-2);
	parentString += (useFullLabel ? "_" :"-")+substate;
	//if(InternalNodeSet.isSubstateInternal(node.getState(),substate))
	//  parentString += "*";
    ...
    public Tree<String> getBestViterbiDerivation(Tree<StateSet> tree, boolean outputScore, boolean useFullLabel){
  	doViterbiInsideScores(tree);
  	if (tree.getLabel().getIScore(0)==Double.NEGATIVE_INFINITY) {
	    //  		System.out.println("Tree is unparsable!");
	    return null;
  	}
  	return extractBestViterbiDerivation(tree, 0, outputScore, useFullLabel);
    }

    public Tree<String> getBestViterbiDerivation(Tree<StateSet> tree, boolean outputScore){
	return getBestViterbiDerivation(tree,outputScore,false);
    }
