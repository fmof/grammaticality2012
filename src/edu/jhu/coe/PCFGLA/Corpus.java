...
public class Corpus {
    ...
    private boolean applyUMap = false;
     ...
     */
    public Corpus(String path, TreeBankType treebank, double fraction, boolean onlyTest, boolean aum) {
	this(path, treebank, fraction, onlyTest, -1,aum);
    }
	
    public Corpus(String path, TreeBankType treebank, double fraction, boolean onlyTest, int skipSection, boolean aum) {
	this(path, treebank, onlyTest, skipSection,aum);
    ...
    public Corpus(String trainList, String validationList, double fraction, boolean aum){
	applyUMap=aum;
	try{
	    loadTrees(trainList, validationList);
	    int beforeSize = trainTrees.size();
	    int endIndex = (int)Math.ceil(beforeSize * fraction);
	    trainTrees = new ArrayList<Tree<String>>(trainTrees.subList(0,endIndex));
	    int nTrainingWords = 0;
	    ...
	    System.err.println("in loading corpus: " + e.getMessage());
	    System.exit(-1);
	}
    }    
    
    public Corpus(String trainList, String validationList, boolean aum){
	applyUMap=aum;
	try{
	    loadTrees(trainList, validationList);
	    int beforeSize = trainTrees.size();
	    int endIndex = (int)Math.ceil(beforeSize);
	    trainTrees = new ArrayList<Tree<String>>(trainTrees.subList(0,endIndex));
	    int nTrainingWords = 0;
	    for (Tree<String> tree : trainTrees) {
		nTrainingWords += tree.getYield().size();
	...
	this(path,treebank,fraction,onlyTest,skipSection,false);
    }

    
    public Corpus(String trainList, String validationList, double fraction){
	this(trainList,validationList,fraction,false);
    }    
    
    public Corpus(String trainList, String validationList){
	this(trainList,validationList,false);
    }    


    private void loadTrees(String trainList, String validationList) throws Exception {
	if(verbose)
	    System.out.println("Loading training trees from "+trainList+"...");
	...
	if(verbose)
	    System.out.println("Loading validation (and also devTest) trees from "+validationList+"...");
        devTestTrees.addAll(readTrees(validationList, Charset.forName("UTF-8")));
        validationTrees.addAll(readTrees(validationList, Charset.forName("UTF-8")));
     ...
     */
    private Corpus(String path, TreeBankType treebank, boolean onlyTest, int skipSection, boolean aum) {
	applyUMap=aum;
	myTreebank = treebank;
	...
	//load from at least one corpus
	    ...
	    try {
		    ...
		    loadSingleFile(path);
    		} else if(myTreebank==TreeBankType.KOREAN){
		    if(verbose)
			System.out.println("Loading Korean converted data!");
		    loadKorean2(path);
		}
	    } catch (Exception e) {
    ...
    public void useUniversalTags(boolean b){
	applyUMap=b;
    }

    private void loadKorean2(String pathKorean) throws Exception {
        System.out.print("Loading Korean treebank v2.0 trees...");
        trainTrees.addAll(readTrees(pathKorean,302000,316999,Charset.forName("UTF8")));
        validationTrees.addAll(readTrees(pathKorean,316000,316999,Charset.forName("UTF8")));
        devTestTrees.addAll(readTrees(pathKorean,317000,317999,Charset.forName("UTF8")));
        finalTestTrees.addAll(readTrees(pathKorean,320000,320999,Charset.forName("UTF8")));
        System.out.print(""
    ...
    public List<Tree<String>> readTrees(String listFile, Charset charset) throws Exception {
	return readTrees(listFile, charset, applyUMap);
    }
    
    public static List<Tree<String>> readTrees(String listFile, Charset charset, boolean aum) throws Exception {
	Collection<Tree<String>> trees = null;
        java.io.File corpusPath = new java.io.File(listFile);
        if (corpusPath.isDirectory()) {
            trees = edu.jhu.coe.io.PennTreebankReader.readTrees(listFile,charset);
        } else {
            try {
                trees = edu.jhu.coe.io.PennTreebankReader.readTrees(listFile,charset);
            } catch (Exception e) {
	...
	Trees.UniversalTagSetTransformer utTrans = new Trees.UniversalTagSetTransformer();
        ...
        for (Tree<String> tree : trees) {
            ...
            Tree<String> normalizedTree = treeTransformer.transformTree(tree);
	    normalizedTreeList.add(aum?utTrans.transformTree(normalizedTree):normalizedTree);
        }
        if (normalizedTreeList.size()==0) {
    ...
    public List<Tree<String>> readTrees(String basePath, int low, int high, Charset charset) throws Exception{
	return readTrees(basePath,low,high,charset,applyUMap);
    }

    public static List<Tree<String>> readTrees(String basePath, int low, int high, Charset charset, boolean aum) throws Exception {
