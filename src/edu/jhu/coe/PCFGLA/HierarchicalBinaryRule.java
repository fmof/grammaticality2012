...
public class HierarchicalBinaryRule extends BinaryRule {
	...
	scoreThisLevel[0][0][0] = Math.log(b.getScore(0,0,0));
	...
	double[][][] scores = new double[newLStates][newRStates][newPStates];
	for (int level=0; level<=lastLevel; level++){
		...
		for (int rChild=0; rChild<newRStates; rChild++){
			...
			scores[lChild][rChild][parent] += scoresThisLevel[lChild/divisorL][rChild/divisorR][parent/divisorP];
		    }
		}
	    }
	}
	for (int lChild=0; lChild<newLStates; lChild++){
	    for (int rChild=0; rChild<newRStates; rChild++){
		for (int parent=0; parent<newPStates; parent++){
		    scores[lChild][rChild][parent] = Math.exp(scores[lChild][rChild][parent]);
		}
	    }
	}
	setScores2(scores);
