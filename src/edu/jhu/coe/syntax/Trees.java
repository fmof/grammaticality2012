...
import edu.jhu.coe.util.CollectionUtils;
import edu.jhu.coe.util.Filter;
...
public class Trees {
    ...
    public static class UniversalTagSetTransformer implements TreeTransformer<String> {
	public Tree<String> transformTree(Tree<String> tree) {
	    return transformTreeHelper(tree);
	}

	private Tree<String> transformTreeHelper(Tree<String> tree) {
	    final String label = UniversalTagMapper.get(tree.getLabel());
	    if (label.equals("-NONE-")) {
		return null;
	    }
	    if (tree.isLeaf()) {
	...
	private Tree<String> readTree(boolean isRoot) throws IOException {
		...
		return isRoot ? new NamedTree<String>(label.toLowerCase().intern(), children, currTreeName) : new Tree<String>(label.toLowerCase().intern(), children);
	    }
	}
	...
	return max;
        ...
        /* These hash maps map between string and integer representations
     * of nonterminals read in from the treebank.  We want to use
     * Integers for efficient representation and (especially) lookup.
     */
    public static HashMap<String,Integer> labelStringToInt = new HashMap<String,Integer>();
    public static HashMap<Integer,String> labelIntToString = new HashMap<Integer,String>();
    // stores list of preterminals
    public static HashMap<Integer,Boolean> preterminalHash = new HashMap<Integer,Boolean>();
    // stores list of nonterminals
    public static HashMap<Integer,Boolean> nonterminalHash = new HashMap<Integer,Boolean>();

    
    public static Tree<Integer> stringTreeToIntegerTree(Tree<String> oldTree) {
        // create a new entry if it doesn't exist already
        Integer id = labelStringToInt.get(oldTree.getLabel());
        if (id == null) {
            id = labelStringToInt.size() + 1;
            labelStringToInt.put(oldTree.getLabel(), id); 
            labelIntToString.put(id, oldTree.getLabel());
        }

        // important: this needs to be done every time, and not just
        // when we discover new ids, to prevent the case where a label
        // is shared between the nonterminal and terminal sets; in
        // that case, if this block is embedded within the above
        // block, if the terminal is discovered first, it will not be
        // entered into the correct hash
        if (! oldTree.isLeaf()) {
            if (oldTree.isPreTerminal())
                preterminalHash.put(id,true);
            else
                nonterminalHash.put(id,true);
        }

        ArrayList<Tree<Integer>> children = new ArrayList<Tree<Integer>>();
        for (Tree<String> child : oldTree.getChildren()) {
            children.add(stringTreeToIntegerTree(child));
        }

        Tree<Integer> newTree = new Tree<Integer>(id,children);
        newTree.setInternal(oldTree.isInternal());
        newTree.setTerminal(oldTree.isTerminal());

        return newTree;
    }


    public static Tree<String> integerTreeToStringTree(Tree<Integer> oldTree) {

        String label = labelIntToString.get(oldTree.getLabel());
        // TODO: error checking

        ArrayList<Tree<String>> children = new ArrayList<Tree<String>>();
        for (Tree<Integer> child : oldTree.getChildren()) {
            children.add(integerTreeToStringTree(child));
        }
